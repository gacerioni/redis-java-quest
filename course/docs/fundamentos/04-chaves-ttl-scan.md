...
Kaelith fez login. A sessão dela precisa existir pelos próximos 30 minutos e depois sumir sozinha, sem nenhum job de limpeza. No Redis, o prazo de vida é uma propriedade da chave: você cria com `SET ... EX`, consulta com `TTL`, renova com `EXPIRE`. E quando precisa achar chaves pelo nome, você nunca usa `KEYS` em produção: `SCAN` percorre o banco em páginas, sem travar o servidor.
...
Troque `SESSION_SECONDS` no `JedisLab` de `1800` para `5` e, logo depois do `RENAME`, coloque um `Thread.sleep(6000)` seguido de `jedis.exists(rotated)`. Rode e veja o `EXISTS` responder `false` e o `TTL` responder `-2`: a chave morreu sozinha. O `./quest check 100-04` vai falhar com a dica de rodar a lição de novo; volte o valor para `1800` e ele passa.
