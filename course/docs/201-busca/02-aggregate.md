...
O conselho da Vila de Brasa quer o preço médio por raridade, o item mais caro de cada tipo e uma tabela de liquidação com 10% de desconto. Fazer isso na aplicação significa puxar 42 documentos (ou 42 mil) e somar em Java. `FT.AGGREGATE` roda um pipeline no servidor, sobre o mesmo índice da lição anterior: agrupa (`GROUPBY`), reduz (`REDUCE COUNT`, `AVG`, `MAX`), calcula campos (`APPLY`), filtra linhas (`FILTER`), ordena e pagina. Só as linhas finais viajam pela rede.
