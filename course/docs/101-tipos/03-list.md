...
Cinco aventureiros querem entrar na mesma dungeon e a ordem de chegada importa. Uma List do Redis é exatamente isso: uma sequência ordenada em que inserir e remover nas pontas custa O(1), não importa o tamanho. `RPUSH` coloca quem chegou no fim, `LPOP` entrega ao matchmaker quem espera há mais tempo. E `BRPOP` deixa um worker esperar por trabalho sem ficar perguntando em loop, ao custo de prender uma conexão enquanto espera: esse custo é o tema de [Conexões bloqueantes](../102-eventos/04-conexoes-bloqueantes.md).
...
Suba uma `Thread` que faça `RPUSH quest:queue:raid dorian` meio segundo depois de o lab começar a esperar e veja o `BRPOP` voltar na hora com `dorian`, sem esperar o segundo inteiro (o check continua passando: a fila da raid volta a ficar vazia). Depois troque `LPOP` por `RPOP` e note que o matchmaker passa a pegar quem chegou por último: a mesma lista virou uma pilha.
