...
Na Floresta de Cinzas, quem está na zona vê o chat na hora; quem chega depois não vê o que foi dito antes. Pub/Sub no Redis é exatamente isso: `PUBLISH` entrega a mensagem para quem estiver assinando o canal naquele instante e não guarda nada. É a ferramenta certa para avisos ao vivo (chat, presença, invalidação de cache) e a ferramenta errada para qualquer coisa que não pode se perder. A lição mostra os dois lados: uma mensagem perdida de propósito e cinco entregues.
...
Pub/Sub não cria chave: o Browser não mostra nada em `{p}:chat:*`. Use a aba Pub/Sub do Insight (menu lateral), assine `{p}:chat:zone:floresta-de-cinzas` e rode a lição de novo: as mensagens aparecem em tempo real na tela e o `PUBLISH` passa a devolver 2 receptores (a sua thread e o Insight). No Profiler dá para ver o `SUBSCRIBE` de um lado e os cinco `PUBLISH` do outro. Saia do canal no Insight antes de rodar o `check`: a lição espera que a primeira mensagem não tenha ninguém ouvindo.
