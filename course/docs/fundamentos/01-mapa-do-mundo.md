...
Ember Realm tem milhares de jogadores online e tudo que precisa responder em milissegundos (fichas, sessões, ranking, chat) mora no Redis. Antes de escrever a primeira linha de código, vale saber onde esse Redis mora: em um processo só, em vários shards que dividem as chaves entre si, ou atrás de um proxy no Redis Cloud. Essa resposta decide qual classe de client você usa, e errar aqui é o tropeço mais comum do primeiro dia: apontar um client de cluster para um banco que não fala cluster.
