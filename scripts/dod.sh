...
fail=0
echo "== 1/4 lint";        scripts/lint_text.sh || fail=1
echo "== 2/4 unit tests";  ./mvnw -q -o -B test 2>&1 | grep -E 'Tests run:.*Fail|BUILD|ERROR' | tail -3; [ "${PIPESTATUS[0]}" -eq 0 ] || fail=1
echo "== 3/4 gauntlet";    scripts/smoke_all.sh || fail=1
echo "== 4/4 site";        scripts/site.sh build 2>&1 | grep -E 'WARNING|ERROR|Documentation built' | tail -5; [ "${PIPESTATUS[0]}" -eq 0 ] || fail=1
