...
Cada golpe do diário gera loot, e o cálculo é pesado demais para um worker só. Com `XREAD` todo leitor vê tudo; o que você quer é dividir: cada golpe para um worker, nenhum golpe esquecido se um worker cair. Consumer groups fazem isso dentro do Redis: o grupo guarda até onde entregou, cada entrega fica pendente até o `XACK`, e `XPENDING` mais `XAUTOCLAIM` recuperam o que um worker morto deixou pela metade. É a base de uma fila de trabalho confiável com Redis.
