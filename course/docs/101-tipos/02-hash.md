...
A ficha de Kaelith tem nome, classe, nível, HP, mana, ouro, XP, zona e guilda. Guardar isso como uma String JSON obriga a ler e reescrever o documento inteiro para mudar um campo; guardar uma chave por atributo espalha a ficha e multiplica viagens. O Hash fica no meio: uma chave, vários campos, cada campo lido, escrito ou incrementado sozinho, no servidor. E desde o Redis 7.4 um campo pode ter prazo próprio: o buff de velocidade expira, o resto da ficha fica.
...
!!! note "Por que o lab recoloca o ouro do seed antes de somar"
    O lab começa com `HSET gold 8400` (o valor do seed) e só então faz `HINCRBY gold 250`. Assim ele pode rodar quantas vezes você quiser e o resultado é sempre 8650, que é o que o check confere. Em produção você não faria isso; é só para a lição ser repetível.
...
Dê prazo ao `shield` também, com `HEXPIRE ... FIELDS 2 haste shield` (Jedis: `hexpire(buffs, 30, "haste", "shield")`; Lettuce: `hexpire(buffs, 30, "haste", "shield")`), e observe no Insight a chave `quest:player:kaelith:buffs` desaparecer inteira quando os dois campos vencerem. Rode o check depois dos 30 segundos e veja o que ele diz. Depois experimente `HINCRBYFLOAT` num campo novo `luck` com `0.5` e veja o valor voltar como texto decimal.
