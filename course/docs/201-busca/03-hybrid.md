...
Um jogador digita "uma arma para mago iniciante". Texto puro (`@type:{arma}`) devolve as 15 armas sem saber qual delas responde à pergunta; busca vetorial pura (KNN sobre o embedding) entende a intenção, mas devolve um manto e uma poção porque "parecem" com a frase. `FT.HYBRID` (Redis 8.4 ou mais novo) roda as duas buscas no servidor e funde os rankings com RRF: quem vai bem nas duas listas sobe. Os embeddings dos 42 itens já estão no JSON (`$.embedding`, 384 dimensões, modelo all-minilm) e a pergunta vem com o vetor pré-computado, então a lição roda sem nenhum modelo instalado.
